    ...
    configs.forEach((config, index) => {
      ...
      cache.set(
        ...
        statusBarDataFromRecentRequests(
          getProviderRecentBuckets(usageByProvider, 'trae', config.apiKey, config.baseUrl)
        )
      );
    });
      ...
      <Card
        title={
          <span className={styles.cardTitle}>
            ...
            {t('ai_providers.trae_title', { defaultValue: 'Trae API 配置' })}
          </span>
        }
        extra={
          <Button size="sm" onClick={onAdd} disabled={actionsDisabled}>
            {t('ai_providers.trae_add_button', { defaultValue: '添加配置' })}
          </Button>
        }
      >
        <ProviderList<ProviderKeyConfig>
          ...
          emptyTitle={t('ai_providers.trae_empty_title', { defaultValue: '暂无Trae配置' })}
          emptyDescription={t('ai_providers.trae_empty_desc', { defaultValue: '点击上方按钮添加第一个配置' })}
                ...
                {item.models?.length ? (
                  <div className={styles.modelTagList}>
                    <span className={styles.modelCountLabel}>
                      {t('ai_providers.trae_models_count', { defaultValue: '模型数量' })}: {item.models.length}
                    </span>
                    {item.models.map((model) => {
                      const configName = model.configName;
                      const modelName = model.modelName;
                      return (
                        <span key={model.name} className={styles.modelTag}>
                          <span className={styles.modelName}>{model.name}</span>
                          {configName && (
                            <span className={styles.modelAlias}>{configName}</span>
                          )}
                          {modelName && (
                            <span className={styles.modelAlias}>{modelName}</span>
                          )}
                        </span>
                      );
                    })}
